 ...
 *
 * @param eventId
 *            이벤트의 고유 식별자 (디버깅, 멱등성 검증, 메시지 추적 용도)
 * @param time
 *            이벤트가 실제 발생한 런타임 시각 (글로벌 서비스 시 TimeZone 동기화의 기준점)
 * @param type
 *            이벤트의 종류를 나타내는 명시적 Enum 값
 * @param payload
 *            실제 비즈니스 데이터를 담고 있는 도메인 이벤트 객체
 * @param <T>
 *            {@link DomainEvent}를 구현한 실제 페이로드 클래스 타입
 */
